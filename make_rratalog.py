...
def error_string_dec(value,err):
    ...
    if type(value)==str:
        ...
        if value.split('.')[-1][-1*int(n_sigfigs)-1]=='0' and not len(value.split('.')[-1])==1:
            #print(value.split('.'))#[-1*int(n_sigfigs)-2])
            zeroflag = 1
        else:
            zeroflag=0
        value = float(value)
...
def error_string_coords(coord,err):
    ...
    if "00:00:" in err:
        ...
        if '.' in new_error:
            new_new_error = new_error.split('.')[-1]
            if int(new_new_error) >= 10:
                #truncate leading zeroes of new_new_error
                new_new_error=new_new_error.lstrip('0')
                endstring = new_value + '(' + new_new_error + ')'
                c_zeroflag = 0
            else:
...
table_keys = ["Name", "RA", "Dec", "DM", "Period", "Pdot", "Frequency", "Fdot", "Bsurf", "Edot", "Tau", "l", "b", "BurstRate"]#, "Flux", "Width"]
...
if __name__ == "__main__":
    ...
    full_df = pd.DataFrame(rrat_dict)
            ...
            for key in table_keys:
                ...
                if key =="Name":
                    ...
                    try:
                        if rrat_toml[key]["error"] == False:
                            ...
                            rrat_dict[key].append(err_string)
                            rrat_dict[key+"_ref"].append(rrat_toml[key]["ref"])
                    except KeyError:
                        rrat_dict[key].append('--')
                        rrat_dict[key+"_ref"].append('--')
                    ...
                    try:
                        ...
                        if rrat_toml[key]["error"] == False:
                            ...
                            rrat_dict[key].append(err_string)
                            #print(str(rrat_toml[key]["value"]) + '  ' + str(rrat_toml[key]["error"]))
                            rrat_dict[key+"_ref"].append(rrat_toml[key]["ref"])
                    except KeyError:
                        rrat_dict[key].append('--')
                        rrat_dict[key+"_ref"].append('--')
                    ...
                    try:
                       z = rrat_toml[key]
                       if rrat_toml[key]["error"] == False:
                           rrat_dict[key].append(p1*1e15)
                           rrat_dict[key+"_ref"].append(rrat_toml["Pdot"]["ref"])
                       else:
                    ...
                    try:
                        ...
                        if rrat_toml["Period"]["error"] == False:
                            ...
                            rrat_dict[key].append(f0)
                            rrat_dict[key+"_ref"].append(rrat_toml["Period"]["ref"])
                        else:
                ...
                #Timing derived parameters from P/Pdot
                    ...
                    try:
                        ...
                        rrat_dict[key].append(np.round(tau,2))
                        rrat_dict[key+"_ref"].append(rrat_toml["Pdot"]["ref"])
                    ...
                    try:
                        ...
                        rrat_dict[key].append(np.round(bsurf,2))
                        rrat_dict[key+"_ref"].append(rrat_toml["Pdot"]["ref"])
                    except KeyError:
                        rrat_dict[key].append('--')
                        rrat_dict[key+"_ref"].append('--')
                    ...
                    try:
                        ...
                        rrat_dict[key].append(np.round(edot,2))
                        rrat_dict[key+"_ref"].append(rrat_toml["Pdot"]["ref"])
                    except KeyError:
                        rrat_dict[key].append('--')
                        rrat_dict[key+"_ref"].append('--')
                ...
                #Derived parameters from position (and DM)
                     ...
                     rrat_dict[key].append(np.round(c_gal.l.deg,2))
                     rrat_dict[key+"_ref"].append(rrat_toml["RA"]["ref"])

                elif key=="b":
                     rrat_dict[key].append(np.round(c_gal.b.deg,2))
                     rrat_dict[key+"_ref"].append(rrat_toml["Dec"]["ref"])
                ...
                #Nested dictionaries
                    ...
                    try:
                        ...
                        try:
                            ...
                            rrat_dict[key].append(z)
                            rrat_dict[key+"_ref"].append(rrat_toml[key]["Discovery"]["ref"])
                        except:
                    ...
                    try:
                        ...
                        rrat_dict[key].append(z)
                        rrat_dict[key+"_ref"].append(rrat_toml[key]["1400"]["ref"])
                    except KeyError:
                        rrat_dict[key].append('--')
                        rrat_dict[key+"_ref"].append('--')                        
    ...
    with open("rratalog.html", "w") as f:
        out = template.render(header=table_keys, units=units, df=full_df)
        f.write(out)
